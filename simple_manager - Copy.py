...
def trigger_one_time_auto_admins(launched_pid=None):
    ...
    admin_names = [
        str(n).strip() for n in app_settings.get("auto_admin_names", []) if str(n).strip()
    ]
    if not admin_names:
        auto_admin_log("Skipped: auto-admin list empty.")
        return

    auto_admin_log(f"Started. Names: {admin_names} | launched PID: {launched_pid}")
    ...
    if not main_hwnd or not input_box:
        auto_admin_log("Gave up: window or input box not found.")
        return

    try:
        settle = float(app_settings.get("auto_admin_delay", AUTO_ADMIN_SETTLE_SECONDS))
    except (TypeError, ValueError):
        settle = AUTO_ADMIN_SETTLE_SECONDS
    ...
    for name in admin_names:
        if not win32gui.IsWindow(main_hwnd):
            break
        box = find_input_box(main_hwnd) or input_box
        msg = f"!admin {name}"
        try:
            send_console_line(box, msg)
            auto_admin_log(f"Sent: {msg}")
        except Exception as e:
            auto_admin_log(f"FAILED to send '{msg}': {e}")
        time.sleep(1.5)
...
class PiBoSoServerManager:
    ...
    def __init__(self, root):
        ...
        BG_DARK = "#181825"        # Base dark background
        FRAME_BG = "#1e1e2e"       # Card / Labelframe background
        TEXT_COLOR = "#cdd6f4"     # Soft readable white text
        ACCENT_COLOR = "#89b4fa"   # Soft blue accent
        ENTRY_BG = "#313244"       # Input field background
        BUTTON_BG = "#45475a"      # Button idle background
        BUTTON_HOVER = "#585b70"   # Button hover background

        self.root.configure(bg=BG_DARK)

        # Global Widget Styles
        style.configure(".", background=BG_DARK, foreground=TEXT_COLOR, font=("Segoe UI", 9))
        style.configure("TFrame", background=BG_DARK)
        style.configure("TLabelframe", background=FRAME_BG, bordercolor="#313244", borderwidth=1)
        ...
        style.map("TCheckbutton", background=[("active", BG_DARK)], foreground=[("active", ACCENT_COLOR)])
        style.map("TRadiobutton", background=[("active", BG_DARK)], foreground=[("active", ACCENT_COLOR)])

        # Entry & Combobox
        style.configure("TEntry", fieldbackground=ENTRY_BG, foreground=TEXT_COLOR, bordercolor="#45475a", insertcolor=TEXT_COLOR)
        style.configure("TCombobox", fieldbackground=ENTRY_BG, background=BUTTON_BG, foreground=TEXT_COLOR, arrowcolor=TEXT_COLOR)
        style.map("TCombobox", fieldbackground=[("readonly", ENTRY_BG)], foreground=[("readonly", TEXT_COLOR)])

        # Buttons
        style.configure("TButton", background=BUTTON_BG, foreground=TEXT_COLOR, borderwidth=0, padding=6, font=("Segoe UI", 9, "bold"))
        style.map("TButton", background=[("active", BUTTON_HOVER)], foreground=[("active", "#ffffff")])

        # Notebook Tabs
        style.configure("TNotebook", background=BG_DARK, borderwidth=0)
        style.configure("TNotebook.Tab", background=FRAME_BG, foreground=TEXT_COLOR, padding=[12, 6], font=("Segoe UI", 9))
        style.map("TNotebook.Tab", background=[("selected", ACCENT_COLOR)], foreground=[("selected", "#11111b")])
        ...
        self.avail_listbox = self.create_styled_listbox(avail_frame, height=9)
        ...
        self.rotation_listbox = self.create_styled_listbox(rotation_frame, height=9)
        ...
        self.specific_bike_listbox = self.create_styled_listbox(specific_list_frame, height=4)
        ...
        self.ann_listbox = self.create_styled_listbox(ann_list_frame, height=8)
        ...
        ttk.Label(
            f_admin_tab,
            text="Add exact player usernames. When the server boots up, it sends '!admin username' once straight away:",
            font=("Segoe UI", 9, "bold"),
        ).pack(anchor="w", pady=(2, 8))
        ...
        self.admin_listbox = self.create_styled_listbox(admin_list_frame, height=8)
        ...
        blacklist_listframe = ttk.Frame(f_blacklist)
        blacklist_listframe.pack(fill="both", expand=True)

        self.blacklist_listbox = self.create_styled_listbox(blacklist_listframe, height=10)
        self.blacklist_listbox.pack(side="left", fill="both", expand=True)

        self.add_entry(f_blacklist, "Unban Reason:", self.unban_reason_var)

        ttk.Button(
            blacklist_listframe, text="Unban Selected", command=self.unban_player
    ...
    # ================= HELPER FOR DARK LISTBOXES =================

    def create_styled_listbox(self, parent, height=8):
        return tk.Listbox(
            parent, height=height, selectmode=tk.SINGLE, exportselection=False,
            bg="#313244", fg="#cdd6f4", selectbackground="#89b4fa", selectforeground="#11111b",
            relief="flat", highlightthickness=1, highlightbackground="#45475a", highlightcolor="#89b4fa"
        )
    ...
    def save_path_settings(self):
        ...
        messagebox.showinfo(
            "Saved",
            "Paths saved. The manager will now use the selected GP Bikes server and mods folder.",
        )
...
"""
    ...
    def save_and_start(self):
        try:
            ...
            messagebox.showinfo(
                "Success",
                "Settings saved, Server launched, and Auto-Admin one-time announcement scheduled!",
            )
